...
public class TileManager {
    ...
    public TileManager(GamePanel gp){
        this.gp = gp;
        tile = new Tile[10];
        mapTileNum = new int[gp.maxWorlCol][gp.maxWorlCol];
        getTileImage();
        loadMap("../map/map.txt");
    }
    ...
    public void loadMap(String map){
        try {
            ...
            while(col < gp.maxWorlCol && row < gp.maxWorlRow){
                String line = br.readLine();
                while(col < gp.maxWorlCol){
                    String numbers[] = line.split(" ");
                    int num = Integer.parseInt(numbers[col]);
                    mapTileNum[col][row] = num;
                    col++;
                }
                if(col == gp.maxWorlCol){
                    col = 0;
                    row++;
                }
            }
    ...
    public void draw(Graphics2D g2) {
        ...
        while (worldCol < gp.maxWorlCol && worldRow < gp.maxWorlRow) {
            int tileNum = mapTileNum[worldCol][worldRow];
    
            int worldX = worldCol * gp.tileSize;
            int worldY = worldRow * gp.tileSize;
    
            // Ajusta para o deslocamento da câmera
            int screenX = worldX - gp.player.worldX + gp.player.screenX;
            int screenY = worldY - gp.player.worldY + gp.player.screenY;
    
            // Apenas desenhar se o tile estiver visível na tela
            if (worldX + gp.tileSize> gp.player.worldX - gp.player.screenX && worldX - gp.tileSize < gp.player.worldX + gp.player.screenX && worldY + gp.tileSize> gp.player.worldY - gp.player.screenY && worldY - gp.tileSize < gp.player.worldY + gp.player.screenY) {
                g2.drawImage(tile[tileNum].image, screenX, screenY, gp.tileSize, gp.tileSize, null);
            }
    
            worldCol++;
            if (worldCol == gp.maxWorlCol) {
                worldCol = 0;
                worldRow++;
            }
        }
